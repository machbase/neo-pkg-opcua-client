...
**값 정규화 (`_normalizeValue`):** `boolean` → 1/0, 그 외 → `Number(value)` 강제 변환. 노드에 `formula`가 있으면 해당 JS 표현식으로 변환 (`value`가 raw 숫자값). formula 없으면 그대로 반환. formula 컴파일은 생성자에서 한 번만 수행하며, 컴파일 실패 시 warn 로그 후 pass-through.
...
| `opcua.nodes[].formula` | 값 변환 JS 표현식 (`value`는 raw 숫자값). 예: `"(value + 100) * 0.001"` |
