...
크기 기반 로테이션 파일 로거. 10 MB 초과 시 현재 파일을 datetime으로 rename(`repli.2026-04-15T03-42-34-064Z.log`)하고 새 `repli.log` 에 이어씀. `maxFiles` 초과 시 오래된 파일부터 삭제.
...
| `opcua.nodes[].add` / `multiply` | 값 변환 오프셋·배율 |
| `db` | 등록된 DB 서버 이름 |
| `dbTable` | 저장 테이블명 |
