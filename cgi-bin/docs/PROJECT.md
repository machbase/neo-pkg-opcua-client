...
**onChanged 초기값 로드:** `close()` 시 마지막 수집 시각을 `data/{name}.last-time.json`에 저장. `start()` 시 해당 파일을 읽어 `TIME >= lastCollectedAt` 조건으로 DB에서 `onChanged: true` 노드의 마지막 값을 일괄 조회해 `_previousValues`를 초기화. 파일 없거나 쿼리 실패 시 무시하고 빈 상태로 시작.
