...
```json
{
  ...
  "config": {
    "opcua": {
      ...
      "nodes": [
        { "nodeId": "ns=3;i=1001", "name": "sensor.tag1", "add": 0, "multiply": 1.0 }
      ]
    },
    "db": "my-server",
    "dbTable": "TAG",
...
| `config.opcua.nodes[].add` | N | 값에 더할 오프셋. 기본값 `0` |
| `config.opcua.nodes[].multiply` | N | 값에 곱할 배율. 기본값 `1.0` |
...
collector config를 조회합니다. `db.password`는 반환하지 않습니다.
  ...
  "data": {
    ...
    "config": {
      "opcua": {
        ...
        "interval": 5000,
        "readRetryInterval": 100,
        "nodes": [{ "nodeId": "ns=3;i=1001", "name": "sensor.tag1", "add": 0, "multiply": 1.0 }]
      },
      "db": "my-server",
      "dbTable": "TAG",
      "valueColumn": "VALUE",
...
**요청 본문** — `POST /collector`의 `config` 필드와 동일한 구조 (`name` 제외)
...
마지막 수집 성공 시간을 epoch ms로 반환합니다. 아직 수집 전이거나 service가 없으면 `null`입니다.
...
```json
{
  "ok": true,
  "data": [
    { "name": "my-server", "config": { "host": "127.0.0.1", "port": 5656, "user": "sys" } }
  ]
}
```
...
```json
{ "endpoint": "opc.tcp://192.168.1.100:4840", "node": "ns=0;i=85", "nodeClassMask": 0 }
```
